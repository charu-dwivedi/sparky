    ...
    for msg in reversed(utils.get_messages_for_user(user, room_name)['items']):
        if 'text' in msg:
            ...
            if days_limit == None and hours_limit == None and min_limit == None or \
                days_limit != None and abs((datetime.now(pytz.utc) - time).days) <= days_limit or \
                hours_limit != None and abs((datetime.now(pytz.utc) - time).total_seconds()) // 3600 <= hours_limit or \
                min_limit != None and abs((datetime.now(pytz.utc) - time).seconds) // 60 <= days_limit:\
